...
RovLink要求设备能够向控制台返回一些必要的数据，并至少能够接受一部分控制指令，这些指令组成了**RovLink基础指令集**。一旦设备满足基础指令集的要求，便可以使用CoralReef上位机软件对基础数据进行接收和发送测试指令。如果RovLink不强制规定基础指令集外的Opcode使用，但为兼容性考虑，推荐开发者遵循下面所说的帧数据类型约定。同时，若开发者需要定义自己的指令，也可以有效利用三个扩展指令子集
...
时间戳为UNIX时间戳或RTOS系统时钟计数截取后32位。如果当前设备正常工作，工作状态指示为0；如果触发心跳包超时，工作状态指示为1
...
控制指令子集包含机器人控制的基础指令。**要求支持RovLink的设备应当至少能够传输Opcode为 `0x2B` 的数据类型**
...
控制台子集包含控制台内部所需的辅助数据。本指令子集没有任何指令包含在基础指令集内
...
基础算法参数子集包含用于向机器人传递控制算法参数（调参）或机器人向上位机回传当前算法参数所需的数据类型。本指令子集没有任何指令包含在基础指令集内
...
机体状态子集包含当前机器人所处的物理状态参数回报数据。本指令子集没有任何指令包含在基础指令集内
...
特殊硬件控制指令子集包含对特殊扩展模块的控制指令，如数控电源、特殊外设、计算加速模块等等。本指令子集没有任何指令包含在基础指令集内
...
模式切换指令子集包含对机器人作业模式进行基本控制的指令数据。**要求支持RovLink的设备应当至少能够回传Opcode为 `0x71` 的数据类型**
...
额外控制指令子集是控制指令子集的扩展，包含有关作业的控制指令数据。**要求支持RovLink的设备应当至少能够回传Opcode为 `0x81` 到 `0x82` 的数据类型**
...
请求指令子集包含控制台对机器人发起数据返回请求使用的指令帧。本指令子集没有任何指令包含在基础指令集内
...
| 数据功能 | 输出电流采样B1 | 输出电流采样B2 | 输出电流采样B3 |
| -------- | -------------- | -------------- | -------------- |
| 数值范围 | uint16_t       | uint16_t       | uint16_t       |
...
状态监测子集包含对机器人本体电控系统和机械结构运行状态的数据回传。本指令子集没有任何指令包含在基础指令集内
...
扩展指令集C用于用户定义自己的指令帧。本指令子集没有任何指令包含在基础指令集内
...
扩展指令集D用于用户定义自己的回传帧和状态帧。本指令子集没有任何指令包含在基础指令集内
...
扩展指令集E用于用户定义自己的功能帧和心跳包。本指令子集没有任何指令包含在基础指令集内
...
兼容指令子集包含用于和其他指令集（包括但不限于上一代ROV控制协议、MAVLink等）对接兼容的数据。本指令子集没有任何指令包含在基础指令集内
